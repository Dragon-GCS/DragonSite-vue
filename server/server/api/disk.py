...
@router.post("/",
             ...
             response_model=List[UserData],
             response_model_exclude=set(RESPONSE_EXCLUDE_FIELDS))
async def create_folder(path: str = Query(regex=FILE_PATH_REGEX),
                        user: User = Depends(get_user),
                        name: List[str] = Query(regex=FILENAME_REGEX)) -> List[UserData]:
...
async def upload_resource(path: str = Query(regex=FILE_PATH_REGEX),
                          user: User = Depends(get_user),
                          files: List[UploadFile] = File()) -> List[UserData]:
...
async def modify_resource(
        ...
        names: List[str] = Query(
            ...
            description=("When names is empty, rename path to target, "
                         "otherwise move these resources to target"))) -> List[UserData]:
    if names:
